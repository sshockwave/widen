...
fn preserves_success_and_owns_the_original_error() {
    let success: Result<_, Widen<String, ()>> = Result::<_, String>::Ok(42).widen();
    assert_eq!(success.unwrap(), 42);

    let error = String::from("original error");
    let address = error.as_ptr();
    let result: Result<(), Widen<_, ()>> = Err(error).widen();
...
fn marker_adds_no_storage_or_send_sync_requirements() {
    ...
    assert_send_sync::<Widen<u64, Rc<()>>>();
    assert_eq!(size_of::<Widen<u64, Rc<()>>>(), size_of::<u64>());
}

#[test]
fn debug_does_not_require_target_to_implement_debug() {
    struct Target;
    let error = Widen::<_, Target>::new(42);
    assert_eq!(format!("{error:?}"), "Widen(42)");
}
